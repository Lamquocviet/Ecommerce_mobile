      ...
      >
        <View className="px-6 gap-2">
          {cartItems.map((item) => {
            ...
            const isProductAvailable = Boolean(product && typeof product.price === "number");
              ...
              <View key={item._id} className="bg-surface rounded-3xl overflow-hidden ">
                <View className="p-4 flex-row">
                  ...
                  <View className="relative">
                    <Image
                      source={{ uri: product?.images?.[0] ?? "https://via.placeholder.com/112" }}
                  ...
                  <View className="flex-1 ml-4 justify-between">
                    <View>
                      ...
                      <View className="flex-row items-center mt-2">
                        <Text className="text-primary font-bold text-2xl">
                          ${isProductAvailable ? (product.price * item.quantity).toFixed(2) : "0.00"}
                        </Text>
                        <Text className="text-text-secondary text-sm ml-2">
                          ${isProductAvailable ? product.price.toFixed(2) : "0.00"} each
                        </Text>
                      </View>
                    </View>
                    ...
                    <View className="flex-row items-center mt-3">
                      <TouchableOpacity
                        ...
                        activeOpacity={0.7}
                        onPress={() => product && handleQuantityChange(product._id, item.quantity, -1)}
                        disabled={isUpdating || !isProductAvailable}
                      >
                        {isUpdating ? (
                      ...
                      <TouchableOpacity
                        ...
                        activeOpacity={0.7}
                        onPress={() => product && handleRemoveItem(product._id, product.name)}
                        disabled={isRemoving || !isProductAvailable}
                      >
